...
val RedisVersionedHashSpec by testSuite {
    for (api in HashBackend.entries) {
        ...
        testWithRedis("$api ordinary string overwrite cannot replace a versioned hash") {
            val store = api.store(this)
            val initial = store.openHash("delivery", 5.minutes)
            commands.pexpire("delivery", 60_000)
            assertFails {
                when (api) {
            ...
            assertTrue(commands.pttl("delivery") in 1..60_000)
        }
        testWithRedis("$api ordinary expiring string overwrite cannot replace a versioned hash") {
            val store = api.store(this)
            val initial = store.openHash("delivery", 5.minutes)
            commands.pexpire("delivery", 60_000)
            assertFails {
        ...
        testWithRedis("$api ordinary mutation string overwrite cannot replace a versioned hash") {
            val store = api.store(this)
            val initial = store.openHash("delivery", 5.minutes)
            commands.pexpire("delivery", 60_000)
            assertFails {
        ...
        testWithRedis("$api invalid hash expiry leaves storage absent") {
            ...
            assertEquals(0L, commands.exists("delivery"))
        }
        ...
        testWithRedis("$api opening versioned storage rejects an ordinary hash without changing it") {
            commands.hset("delivery", "field", "ordinary")
            assertFails { api.store(this).openHash("delivery", null) }
